#include "cubic_style_precompute.h"
#include "cubic_style_data.h"
#include "cubic_style_single_iteration.h"
#include "cubic_style_local_step.h"
...
#include <iostream>
/*
BORROWED CODE FROM DEFORMATION AND SMOOTHING ASSIGNMENT
...
⌘ ⇧ Z    Redo
  ...
  const auto &update = [&]() {
    ...
    else
    {
      ...
      case ARAP:
      {
        cubic_style_single_iteration(data, s.CU, U);
  ...
  viewer.callback_key_pressed =
      [&](igl::opengl::glfw::Viewer &, unsigned int key, int mod) {
        ...
        case 'K':
        case 'k':
          data.lambda = (key == 'K' ? 10.0 : 0.1) * data.lambda;
          std::cout << "lambda: " << data.lambda << std::endl;
          break;
        case 'M':
        case 'm':
        {
        ...
        case ' ':
          ...
          if (!s.placing_handles && s.CV.rows() > 0)
          {
            ...
            cubic_style_precompute(V, F, b, data);
            std::cout << R << std::endl;
          }
          break;
        default:
